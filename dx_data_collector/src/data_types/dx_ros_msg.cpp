...
, msg_parser_(msg_parser)
{
  ...
  // Copy the data from the message into memory
  data_ = std::make_unique<uint8_t[]>(msg.size());
  std::copy(msg.raw_data(), msg.raw_data() + msg.size(), data_.get());

  // Create a span to access this data
  data_span_ = nonstd::span(data_.get(), msg.size());
...
void rios::data_collector::DxRosMsg::toShapeShifter(topic_tools::ShapeShifter& msg) const
{
  ...
  ros::serialization::OStream stream(data_.get(), data_span_.size());
  msg.read(stream);
}

const std::map<std::string, rios::data_collector::FieldData>& rios::data_collector::DxRosMsg::parsedMsgData()
{
