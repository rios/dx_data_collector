...
int setupPipelines(
  rios::cfg & collection_config, rios::data_collector::RosDataIngestor & ingestor,
  ...
  std::unordered_map<std::string, rios::cfg> & outputters_configured)
{
  ...
  for (const auto & pipeline : collection_config["pipelines"]) {
    ...
    if (!pipeline["formatter"]) {
      ROS_ERROR_STREAM(
        "No 'formatter' defined for pipeline " << pipeline_num << ". It will not be setup.");
      continue;
    }
    if (!pipeline["outputter"]) {
      ROS_ERROR_STREAM(
        "No 'output' defined for pipeline " << pipeline_num << ". It will not be setup.");
      continue;
    }
    ...
    if (!formatters_configured.count(formatter_name)) {
      ROS_ERROR_STREAM(
        "No formatter named " << formatter_name
                              << " has been defined in config file 'formatters' section. Typo?");
      continue;
    }

    if (!outputters_configured.count(outputter_name)) {
      ROS_ERROR_STREAM(
        "No outputter named " << outputter_name
                              << " has been defined in config file 'output' section. Typo?");
      continue;
    }
    ...
    ros::param::set(
      "/data_collectors" + ros::this_node::getName() + "/" + formatter_name + "_to_" +
        outputter_name,
      false);

    rios::data_collector::DataFormatter * formatter =
      cpp_formatters.at(formatter_type)(formatters_configured[formatter_name]);
    rios::data_collector::DataOutputter * outputter =
      cpp_outputters.at(outputter_type)(outputters_configured[outputter_name]);

    const auto topic_ingestors =
      std::make_shared<const rios::data_collector::IngestorMap>(ingestor.getTopicIngestors());

    ingestor.registerStoreCallback(
      [formatter_name, outputter_name, formatter, outputter, &collection_config, topic_ingestors](
        std::deque<rios::data_collector::DxRosMsg::Ptr> data,
        std::unordered_map<std::string, std::shared_ptr<std::string>> params,
        std::string snapshot_name) {
        // Start a thread to do data formatting and output
        ROS_INFO_STREAM(
          "[" << formatter_name << " -> " << outputter_name << "] Data store beginning for "
              << data.size() << " unfiltered messages. Snapshot: " << snapshot_name);

        std::thread([formatter_name, outputter_name, formatter, outputter, &collection_config, data,
                     params, snapshot_name, &topic_ingestors]() mutable {
          // Format the data
          auto start = std::chrono::steady_clock::now();
          rios::data_collector::FormattedData::Ptr formatted_data;
          try {
            formatted_data = formatter->formatData(data, params, snapshot_name, topic_ingestors);
          } catch (const std::exception & e) {
            ROS_ERROR_STREAM(
              "[" << formatter_name << "] Formatting failed with exception: " << e.what()
                  << ". Exiting pipeline.");
            return;
          }
          ...
          bool out_success;
          try {
            out_success = outputter->outputData(
              collection_config["episode_name"].as<std::string>(), formatted_data, error_str);
          } catch (const std::exception & e) {
            ROS_ERROR_STREAM(
              "[" << outputter_name << "] Outputting failed with exception: " << e.what());
            return;
          }
          ...
          if (out_success) {
            ROS_INFO_STREAM(
              "[" << outputter_name << "] Outputting succeeded in " << elapsed << "ms");
          } else {
            ROS_ERROR_STREAM(
              "[" << outputter_name << "] Outputting failed in " << elapsed
                  << "ms with error: " << error_str);
          }
        }).detach();
      });

    ROS_INFO_STREAM(
      "Successfully set up pipeline [ROS] -> [" << formatter_name << "] -> [" << outputter_name
                                                << "]");
    ros::param::set(
      "/data_collectors" + ros::this_node::getName() + "/" + formatter_name + "_to_" +
        outputter_name,
